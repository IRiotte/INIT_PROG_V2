...
public class AppliDessin extends Application {
    ...
    */
    ...
    public void augmenteSlider(){
        double valeur = this.slider.getValue();
        double max = this.slider.getMax();
        if (valeur + 1 <= max){
            this.slider.setValue(valeur + 1);
        }
        else { this.slider.setValue(max); }
        
    }

    public void diminueSlider(){
        double valeur = this.slider.getValue();
        double min = this.slider.getMin();
        if (valeur - 1 >= min){
            this.slider.setValue(valeur - 1);
        }
        else {this.slider.setValue(min);}
        
    }

    public void chanegCouleurCercle(){
        Circle c = (Circle) this.boite.getChildren().get(this.boite.getChildren().size()-1);
